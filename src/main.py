...
from embedding import Embedder, VectorStoreBuilder, MMRDocumentRetriever
...
llm_path = huggingface_paths.llm_path

local_paths = LocalPaths()
vector_store_dir_path = local_paths.vector_store_dir_path


categories = ['pathology', 'neuroscience', 'paleontology']
n_files = 4
...
# mmr_document_retriever = MMRDocumentRetriever(embeddings, vector_store_dir_path, 5)
# docs = mmr_document_retriever.retrieve_document(query=query)
# print(docs)

# large_language_model = LargeLanguageModel(llm_path=llm_path, use_api=True)
# llm = large_language_model.set_llm()

# llm_output = llm.invoke('Hello')
# print(llm_output)
